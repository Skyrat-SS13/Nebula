...
/obj/item/clothing/update_icon()	// picks the colored overlays from the ICON file
	...
	if(hasprimary)	//Checks if the overlay is enabled
		...
		overlays += primary_overlay	//Applies the coloured overlay onto the item sprite. but NOT the mob sprite.
	if(hassecondary)
		...
		switch(choice)	//Lets the list's options actually lead to something
			if("Primary Color")
				...
				if(primary_color_input)	//Checks if the color selected is NULL, rejects it if it is NULL.
					primary_color = sanitize_hexcolor(primary_color_input, primary_color)	//formats the selected color properly
			...
			if("Secondary Color")
				...
				if(secondary_color_input)
					secondary_color = sanitize_hexcolor(secondary_color_input, secondary_color)
				update_icon()
				user.regenerate_icons()
			if("Tertiary Color")
				var/tertiary_color_input = input(usr,"","Choose Tertiary Color",tertiary_color) as color|null
				if(tertiary_color_input)
					tertiary_color = sanitize_hexcolor(tertiary_color_input, tertiary_color)
				update_icon()
				user.regenerate_icons()
