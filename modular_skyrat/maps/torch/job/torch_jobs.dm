/datum/map/torch
	species_to_job_whitelist = list(
		/datum/species/adherent = list(/datum/job/ai, /datum/job/cyborg, /datum/job/assistant, /datum/job/janitor, /datum/job/chef, /datum/job/bartender, /datum/job/cargo_tech,
										...
										/datum/job/mining),
		/datum/species/serpentid = list(/datum/job/ai, /datum/job/cyborg, /datum/job/janitor, /datum/job/scientist_assistant, /datum/job/chemist,
...
/datum/map/torch/setup_job_lists()
	for(var/job_type in allowed_jobs)
		...
		if(job && (DEPT_COMMAND in job.department_refs) && job.allowed_branches.len && !(/datum/mil_branch/civilian in job.allowed_branches))
...
/*/decl/cultural_info/culture/ascent/Initialize()
	LAZYADD(qualifications, /datum/job/scientist_assistant)
	..()

/decl/cultural_info/culture/ascent/Initialize()
	LAZYADD(qualifications, /datum/job/cargo_tech)
	..()

/decl/cultural_info/culture/ascent/Initialize()
	LAZYADD(qualifications, /datum/job/engineer)
	..()

/decl/cultural_info/culture/ascent/plus/Initialize()
