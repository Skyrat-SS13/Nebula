...
/datum/submap/ascent/check_general_join_blockers(var/mob/new_player/joining, var/datum/job/submap/job)
	...
	if(. && istype(job, /datum/job/submap/ascent))
		...
		if(ascent_job.set_species_on_join == SPECIES_MANTID_GYNE && !is_species_whitelisted(joining, SPECIES_MANTID_GYNE))
			...
			return FALSE
		if(ascent_job.set_species_on_join == SPECIES_MANTID_ALATE && !is_species_whitelisted(joining, SPECIES_SERPENTID))
			to_chat(joining, SPAN_WARNING("You must be whitelisted to play a [SPECIES_SERPENTID] to join as a [SPECIES_MANTID_ALATE]."))
			return FALSE
